...
class DirectionService:
    ...
    def generate(self, project: Project | None = None, documents: list[Document] | None = None) -> dict[str, Any]:
        ...
        return {
            "directions": [
                {
                    "name": "问题驱动型PPT",
                    "scenario": f"适合围绕“{core_question or title}”制作病例讨论、科室教学或疑难问题复盘PPT。",
                    "reason": "先把PPT要回答的问题放清楚，再按资料和证据逐层推进，页面逻辑更强。",
                ...
                {
                    "name": "教学查房型PPT",
                    "scenario": "适合住培教学、带教PPT、病例教学和科室内部培训。",
    ...
    def _generate_with_llm(self, project: Project, documents: list[Document]) -> dict[str, Any] | None:
        ...
        result = LLMService(self.settings).chat_json(
            "你是医学PPT制作策划助手。只输出严格 JSON，不输出个体化诊疗建议。",
            f"""
请为医学PPT制作生成 3 个可选制作方案，输出 JSON：
